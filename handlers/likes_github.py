"""
Free Fire Likes - GitHub Method (FAST & WORKING)
Uses proven open-source method from GitHub
"""
...
from github_likes_api import send_likes_github_method, get_player_name_real
...
async def likes_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Handle /likes command - GITHUB METHOD - FAST"""
    ...
    try:
        ...
        player_info = await get_player_name_real(target_uid)
        ...
        await msg.edit_text(
            ...
            f"⚡ Fast delivery (20-40 seconds)..."
        )

        # Send likes using GitHub method
        result = await send_likes_github_method(target_uid, LIKES_TO_SEND)
        ...
        if successful == 0:
            await msg.edit_text(
                "❌ No likes sent!\n\n"
                "Guest accounts could not authenticate.\n\n"
                "💰 No coins deducted."
            )
            return
        ...
        # Get updated likes
        player_after = await get_player_name_real(target_uid)
        ...
        success_msg += f"⚡ GitHub Open-Source Method"

        await msg.edit_text(success_msg)
        logger.info(f"✅ GitHub method: Sent {successful} likes to {player_name} ({target_uid})")
