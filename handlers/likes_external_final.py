"""
Free Fire Likes - EXTERNAL SERVICE METHOD
Uses proven third-party APIs - NO guest account creation needed
...
from external_likes_service import get_player_info_from_ffinfo, send_likes_via_api
...
async def likes_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Handle /likes command - EXTERNAL SERVICE - 100% WORKING"""
    ...
    try:
        ...
        # Get REAL player info from Garena API
        player_info = await get_player_info_from_ffinfo(target_uid)
        ...
        # Send likes via external API service
        result = await send_likes_via_api(target_uid, LIKES_TO_SEND)

        if not result.get("success"):
            await msg.edit_text(
                "❌ Like service temporarily unavailable!\n\n"
                "The external likes provider is not responding.\n"
        ...
        successful = result.get("sent", 0)
        failed = result.get("failed", 0)

        if successful == 0:
            await msg.edit_text(
                "❌ Failed to send likes!\n\n"
                "The service did not send any likes.\n\n"
        ...
        # Get updated likes
        player_after = await get_player_info_from_ffinfo(target_uid)
        ...
        success_msg += f"🌐 Sent via External Service"

        await msg.edit_text(success_msg)
        logger.info(f"✅ Sent {successful} likes to {player_name} ({target_uid}) via external API")
